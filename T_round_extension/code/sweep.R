...
# ============================================================
#
# main_sweep(): run every config, save raw + summary + plots to disk.
# Use sweep_one("signal_noise") to run a single sweep interactively.

sweep_one <- function(sweep_name, seeds = 1:20, base_params = NULL,
                      out_dir = "sweep_results", save = TRUE) {
  config <- SWEEP_CONFIGS[[sweep_name]]
  cat(sprintf("\n=== Sweep: %s ===\n%s\n\n", config$name, config$description))
  
  raw <- run_sweep(sweep_name, seeds = seeds, base_params = base_params)
  summ <- summarize_sweep(raw, config)
  
  if (save) {
    dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
    saveRDS(raw,  file.path(out_dir, paste0(sweep_name, "_raw.rds")))
    saveRDS(summ, file.path(out_dir, paste0(sweep_name, "_summary.rds")))
    
    p1 <- plot_sweep_from_config(summ, config, "primary")
    p2 <- plot_sweep_from_config(summ, config, "secondary")
    if (!is.null(p1)) ggsave(file.path(out_dir, paste0(sweep_name, "_primary.png")),
                             p1, width = 8, height = 5.5, dpi = 120)
    if (!is.null(p2)) ggsave(file.path(out_dir, paste0(sweep_name, "_secondary.png")),
                             p2, width = 8, height = 5.5, dpi = 120)
    cat(sprintf("Saved to %s/\n", out_dir))
  }
  
  invisible(list(raw = raw, summary = summ, config = config))
}

main_sweep <- function(seeds = 1:20, base_params = NULL,
                       out_dir = "sweep_results") {
  for (name in names(SWEEP_CONFIGS)) {
    sweep_one(name, seeds = seeds, base_params = base_params, out_dir = out_dir)
  }
  cat("\nAll sweeps complete. Files in", out_dir, "\n")
}
